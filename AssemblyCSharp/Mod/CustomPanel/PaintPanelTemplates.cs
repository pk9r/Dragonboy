...
namespace Mod.CustomPanel
{
    internal static class PaintPanelTemplates
    {
        ...
        public static void paintCollectionCaptionAndDescriptionTemplate<T>(Panel panel, mGraphics g, ICollection<T> collection,
            Func<T, string> getCaption, Func<T, string> getDescription, bool captionIndex = true)
        {
            ...
            g.setColor(0);
            if (collection == null || collection.Count != panel.currentListLength) return;
