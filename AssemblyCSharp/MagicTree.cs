	...
	public override void paint(mGraphics g)
	{
		...
		try
		{
			for (int i = 0; i < currPeas; i++)
			{
				...
				if (ModMenuMain.getStatusInt("levelreducegraphics") < 2) g.drawImage(pea, cx + peaPostionX[i] - SmallImage.smallImg[id][3] / 2, cy + peaPostionY[i] - SmallImage.smallImg[id][4], 0);
			}
		}
		catch (Exception)
		{
