...
public class Panel : IActionListener, IChatable
{
		...
		try
		{
			for (int j = 1; j < currentListLength; j++)
			{
				...
				if (item.itemOption != null)
				{
					...
					for (int num10 = 0; num10 < item.itemOption.Length; num10++)
					{
						paintOptSlotItem(g, item.itemOption[num10].optionTemplate.id, item.itemOption[num10].param, num6, num7, num8, num9);
					}
				}
				if (item.quantity > 1)
					mFont.tahoma_7_yellow.drawString(g, "x" + item.quantity, num6 + num8, num7 + num9 - mFont.tahoma_7_yellow.getHeight(), 1);
	...
	{
		mFont.tahoma_7b_white.drawString(g, "Mod Cộng Đồng v" + GameMidlet.VERSION, 60, 4, mFont.LEFT, mFont.tahoma_7b_dark);
		mFont.tahoma_7_yellow.drawString(g, mResources.character + ": " + Char.myCharz().cName, 60, 16, mFont.LEFT, mFont.tahoma_7_grey);
		mFont.tahoma_7_yellow.drawString(g, mResources.account_server + " " + Utilities.server["name"] + ":", 60, 27, mFont.LEFT, mFont.tahoma_7_grey);
	...
	private void doFireSkill()
	{
		...
		int num5 = selected;
		SkillTemplate skillTemplate = Char.myCharz().nClass.skillTemplates[num5 - 6];
		Skill skill = Char.myCharz().getSkill(skillTemplate);
		Skill skill2 = null;
