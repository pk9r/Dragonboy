...
public class TField : IActionListener
{
    ...
    public void paintInputTf(mGraphics g, bool iss, int x, int y, int w, int h, int xText, int yText, string text, string info)
    {
        ...
        if (Utils.IsPC() && selectStartIndex > -1)
        {
            ...
            string selectedText = text.Substring(Math.Min(selectStartIndex, caretPos), Math.Abs(selectStartIndex - caretPos));
            if (inputType == INPUT_TYPE_PASSWORD)
                selectedText = new string('*', selectedText.Length);
            g.fillRect(xText + mFont.tahoma_8b.getWidth(paintedText.Substring(0, Math.Min(selectStartIndex, caretPos))) - 1, yText, mFont.tahoma_8b.getWidth(selectedText), mFont.tahoma_8b.getHeight());
        }
                ...
                case KeyCode.Backspace:
                    ...
                    else
                    {
                        if (text.Length > 0)
                        {
                            ...
                            text = text.Substring(0, Math.Min(caretPos, selectStartIndex)) + text.Substring(Math.Max(caretPos, selectStartIndex));
                ...
                default:
                    if (Event.current.control)
                    {
                        ...
                        else if (Event.current.keyCode == KeyCode.V)
                        {
                            ...
                            {
                                int newCaretPos = Math.Min(caretPos, selectStartIndex);
                                text = text.Substring(0, Math.Min(caretPos, selectStartIndex)) + GUIUtility.systemCopyBuffer + text.Substring(Math.Max(caretPos, selectStartIndex));
